...
/*
    public JSONObject list(String forumShortName, int threadId, String since, int limit, String order) {
        StringBuilder query = new StringBuilder("SELECT id, message, date, likes, dislikes, points, isApproved, isHighlighted, isEdited, isSpam, isDeleted, parent, users_email, threads_id, forums_short_name FROM posts WHERE ");

        if (forumShortName != null) {
            query.append("forums_short_name = ?");
        } else {
        ...
        if (order != null && (order.equals("asc") || order.equals("desc"))) {
            query.append(" ORDER BY date " + order.toUpperCase());
        }
    ...
    private JSONObject listWithStatement(PreparedStatement stmt) {
        JSONObject result = new JSONObject();
        List<JSONObject> response = new ArrayList<JSONObject>();
        ...
        try {
            ...
            while (post.next()) {
                ...
                obj.put("id", post.getString("id"));
                obj.put("message", post.getString("message"));
                obj.put("date", post.getString("date"));
                obj.put("likes", post.getInt("likes"));
                obj.put("dislikes", post.getInt("dislikes"));
                obj.put("points", post.getInt("points"));
                obj.put("isApproved", post.getBoolean("isApproved"));
                obj.put("isHighlighted", post.getBoolean("isHighlighted"));
                obj.put("isEdited", post.getBoolean("isEdited"));
                obj.put("isSpam", post.getBoolean("isSpam"));
                obj.put("isDeleted", post.getBoolean("isDeleted"));
                obj.put("parent", post.getInt("parent"));
                obj.put("user", post.getString("users_email"));
                obj.put("thread", post.getInt("threads_id"));
                obj.put("forum", post.getString("forums_short_name"));
